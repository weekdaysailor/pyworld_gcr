...
class GCRModel(BaseModel):
    ...
    def initialize_model(self) -> None:
        ...
        # Add GCR-specific initializations
        self.world3.set_policy_delay(
            self.reward_start_year - self.start_time,
            policy_year=self.reward_start_year
        )
    ...
    def run_simulation(self) -> pd.DataFrame:
        ...
        for year in range(self.start_time, self.stop_time + 1):
            if year >= self.reward_start_year:
                emissions = results.loc[year, 'industrial_co2_emissions']
